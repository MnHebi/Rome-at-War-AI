...
`civ-strategy-data.json`, constrained by the agreed unit-focus workbook. Lower
difficulty differences are stored in `civ-strategy-historical-overrides.json` and
the model is explained in `HISTORICAL_STRATEGY.md`.
...
- `sync_civ_strategies.py` validates both difficulty profiles and updates every
  civ PER;
- `validate_per.py` checks PER structure, preprocessor balance, and constants;
- `validate_strategy_execution.py` checks unit-role coverage, unit-line actions,
  rush activation/execution, the enemy matchup matrix, and every difficulty
  preprocessor branch; and
